...
class OverridePlugin extends AbstractPlugin
{
    ...
    {
        if ($GLOBALS['MIME'] !== 'text/html' && $GLOBALS['MIME'] !== 'application/javascript' && $GLOBALS['MIME'] !== 'text/css') return;
        if (!$GLOBALS['CACHE_ADAPTER']->isExisting( $GLOBALS['PATTERN_SEARCH_MERGE_KEY'] = base64_encode($GLOBALS['SITE_HOST'] . '_pattern_search_merge'))) $this->pattern_search_merge();
        if (!$GLOBALS['CACHE_ADAPTER']->isExisting($GLOBALS['PATTERN_REPLACE_MERGE_KEY'] = base64_encode($GLOBALS['SITE_HOST'] . '_pattern_replace_merge'))) $this->pattern_replace_merge();
        if (!$GLOBALS['CACHE_ADAPTER']->isExisting($GLOBALS['GLOBAL_SEARCH_MERGE_KEY'] = base64_encode($GLOBALS['SITE_HOST'] . '_global_search_merge'))) $this->global_search_merge();
        if (!$GLOBALS['CACHE_ADAPTER']->isExisting($GLOBALS['GLOBAL_REPLACE_MERGE_KEY'] = base64_encode($GLOBALS['SITE_HOST'] . '_global_replace_merge'))) $this->global_replace_merge();
